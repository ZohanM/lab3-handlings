...
void ST7735_XYplotInit(char *title, int32_t minX, int32_t maxX, int32_t minY, int32_t maxY, uint16_t color){
  ...
  scaleX =  128.0/(maxX-minX);
  scaleY = -128.0/(maxY-minY);
  offsetX = (-1.0 * minX) * scaleX;
  offsetY = (-1.0 * minY) + scaleY + 160;
}
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[],uint16_t color){
  ...
  int newY = 0;
  for(int i = 0; i < num; i++){
    newX = (int)((bufX[i] * scaleX) + offsetX);
    newY = (int)((bufY[i] * scaleY) + offsetY);
